...
test_default_install_layout() {
    ...
    assert_contains "$skills_root/ai-flow-git-commit/SKILL.md" "git stash"
    assert_contains "$skills_root/ai-flow-git-commit/SKILL.md" "Gitmoji"
    ...
    assert_contains "$skills_root/ai-flow-plan/SKILL.md" '必须至少逐项扫描以下决策维度'
    assert_contains "$skills_root/ai-flow-plan/SKILL.md" '每个维度都必须落入以下三种状态之一后，才算 intake 完成'
    assert_contains "$skills_root/ai-flow-plan/SKILL.md" '必须先向用户询问，不能直接把假设写进 plan'
    assert_contains "$skills_root/ai-flow-plan/SKILL.md" '若一次扫描识别出 3 个及以上独立不确定项'
...
test_custom_roots_install() {
    ...
    assert_contains "$onespace_skills/ai-flow-git-commit/SKILL.md" "git stash"
    assert_contains "$onespace_skills/ai-flow-git-commit/SKILL.md" "Gitmoji"
    ...
    assert_contains "$onespace_skills/ai-flow-plan/SKILL.md" '必须至少逐项扫描以下决策维度'
    assert_contains "$onespace_skills/ai-flow-plan/SKILL.md" '每个维度都必须落入以下三种状态之一后，才算 intake 完成'
    assert_contains "$onespace_skills/ai-flow-plan/SKILL.md" '必须先向用户询问，不能直接把假设写进 plan'
    assert_contains "$onespace_skills/ai-flow-plan/SKILL.md" '若一次扫描识别出 3 个及以上独立不确定项'
