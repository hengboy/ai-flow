...
- 定位 executor 时只能在以下绝对候选路径中用 `test -x` 逐个探测：`$HOME/.claude/agents/ai-flow-opencode-plan-review/bin/plan-review-executor.sh`、`$HOME/.config/opencode/agents/ai-flow-opencode-plan-review/bin/plan-review-executor.sh`。禁止使用 `./bin/plan-review-executor.sh`、`bin/plan-review-executor.sh`、`$PWD/...` 或任何用户工作区相对路径。
...
- 当前代理与 `ai-flow-codex-plan-review` 形成配对命名，执行器在需要时可切换配对引擎。

### 固定输出协议
```text
RESULT: success|failed
AGENT: ai-flow-opencode-plan-review
