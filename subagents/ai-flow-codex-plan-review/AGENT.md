...
- 定位 executor 时只能在以下绝对候选路径中用 `test -x` 逐个探测：`$HOME/.claude/agents/ai-flow-codex-plan-review/bin/plan-review-executor.sh`、`$HOME/.config/opencode/agents/ai-flow-codex-plan-review/bin/plan-review-executor.sh`。禁止使用 `./bin/plan-review-executor.sh`、`bin/plan-review-executor.sh`、`$PWD/...` 或任何用户工作区相对路径。
