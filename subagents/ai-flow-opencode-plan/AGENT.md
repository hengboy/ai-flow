...
- 定位 executor 时只能在以下绝对候选路径中用 `test -x` 逐个探测：`$HOME/.claude/agents/ai-flow-opencode-plan/bin/plan-executor.sh`、`$HOME/.config/opencode/agents/ai-flow-opencode-plan/bin/plan-executor.sh`。禁止使用 `./bin/plan-executor.sh`、`bin/plan-executor.sh`、`$PWD/...` 或任何用户工作区相对路径。
...
3. 所有 plan 写入必须落到 `.ai-flow/plans/YYYYMMDD/<slug>.md`；状态只能由 `flow-state.sh` 创建或更新。
