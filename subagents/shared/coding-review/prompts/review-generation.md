...
2. 对比以下实施计划，审查变更是否完整实现了计划中的所有步骤，并检查 Step 的“本轮 review 预期关注面”“本步关闭条件”是否被真实满足
   - `## 2. 计划覆盖度检查` 必须使用 plan 中真实的 `Step ID` 逐条列出覆盖结论；regular / recheck 模式下必须覆盖 plan 的全部 Step，incremental 模式下只允许列出本轮增量实际涉及的 Step，但不得编造、合并或遗漏已纳入本轮范围的 Step
...
10. 如果当前是 incremental 模式（见 `__AI_FLOW_REVIEW_MODE__`），仅审查上次常规审查后新增的 Git 变更（新增文件、修改文件的变更行、新增 untracked 文件），不要重复审查上次报告中已覆盖的内容，除非新增变更直接影响了已审查代码的正确性。在”1.1 审查上下文”中注明增量基准时间和上次审查报告路径，在”2. 计划覆盖度检查”中仅关注增量范围内涉及的实施步骤。增量模式仍需要完整执行”3.5 逻辑正确性”和”3.6 缺陷族覆盖度”，但缺陷族覆盖仅需关注与增量变更相关的缺陷族
11. 首行必须保持 `# 审查报告：__AI_FLOW_PLAN_TITLE__`
12. 你只负责输出完整审查报告正文；绑定 `slug` 时，外层执行器会基于该报告立即调用一次完整的 `flow-state.sh transition` 推进状态。不要在报告中暗示”稍后再补状态”或要求用户手工推进。
