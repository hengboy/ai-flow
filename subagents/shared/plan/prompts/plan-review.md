...
5. **模板结构合规性**（由 `plan-validate.sh validate-template` 预检完成，预检失败已由执行器写入 8.2 并返回 failed；本审核聚焦于需求对齐、可落地性、验证闭环和风险判断。若发现模板结构问题，直接指出，不再逐项校验）：
...
- ITEMS 内容必须可直接写入 plan 的“8.2 偏差与建议”
