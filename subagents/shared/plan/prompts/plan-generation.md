...
20. 如果当前为 workspace 模式（见上方 `__AI_FLOW_WORKSPACE_CONTEXT__`），每个文件路径必须相对于 workspace 根目录；每个”涉及模块”和”文件边界总览”表格必须填写 `仓库` 列，值为 manifest 中声明的 repo id；所有 Git 验证命令必须使用 `git -C <repo_path>` 前缀。单仓模式下 `仓库` 列填写 `-`。
