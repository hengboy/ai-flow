...
| 当前状态 | 操作 |
|----------|------|
| `PLANNED` | `plan_reopened` 转换到 `AWAITING_PLAN_REVIEW` |
| `IMPLEMENTING` | `plan_reopened` 转换到 `AWAITING_PLAN_REVIEW` |
| `AWAITING_REVIEW` | `implementation_reopened` 转换到 `IMPLEMENTING` |
| `DONE` | `implementation_reopened` 转换到 `IMPLEMENTING` |
| `REVIEW_FAILED` | `implementation_reopened` 转换到 `IMPLEMENTING` |
| `FIXING_REVIEW` | `implementation_reopened` 转换到 `IMPLEMENTING` |
...
- 只有在变更导致状态进入 `AWAITING_PLAN_REVIEW` 时，才需要立即重新走 `/ai-flow-plan-review`；如果状态被 `implementation_reopened` 推回 `IMPLEMENTING`，则应先完成本轮变更落地，再按后续阶段进入审核。
