...
- 读取 `.ai-flow/state/{日期}-{slug}.json` 获取 `current_status` 和 `plan_file`
- 读取完整计划文件
- 与用户确认变更内容：
  - 新增了什么需求？
  - 修改了什么已有需求？
...
```bash
${AI_FLOW_HOME:-$HOME/.config/ai-flow}/scripts/flow-state.sh show {日期}-{slug} --field current_status
```
...
```bash
${AI_FLOW_HOME:-$HOME/.config/ai-flow}/scripts/flow-change.sh {日期}-{slug} "[root-cause-review-loop] 根因：...；受影响缺陷族：...；前两轮遗漏原因：...；补充验证：..."
```
...
- 不得修改 `.ai-flow/state/{日期}-{slug}.json` 中的固定 schema 字段（repair 状态转换除外）
- 不得修改已有的审查报告
