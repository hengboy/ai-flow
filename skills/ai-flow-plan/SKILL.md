...
- **长任务判断（计划组）**：如果需求显式包含 `--group` 参数，或需求描述中明确提及"长任务"、"计划组"、"拆分"、"多阶段"等关键词，或需求横跨 3 个以上独立模块/子系统，则判定为长任务：
  - 长任务生成计划组文档（`.ai-flow/plan-groups/{group_slug}.md`），调用 `flow-plan-group.sh create` 创建状态，进入 `AWAITING_GROUP_REVIEW`
  - 长任务不会立即生成任何子计划
...
**先读取最终生效配置（项目级优先，用户级回退）的 `engine_mode`，再决定目标 subagent。不要跳过这一步，也不得替换为任何其他 agent（包括内置 Plan agent）。**
...
当需求被判定为长任务时，不生成单份 plan，而是生成计划组文档：

- 委派 plan subagent 生成计划组文档（包含拆分方案、children[] 定义）
- 计划组文档头部元数据固定格式：
  ```
  > 创建日期：{YYYY-MM-DD}
  > 创建时间：{HH:MM:SS}
  > 需求简称：{需求简称}
  ```
- 调用 `flow-plan-group.sh create` 创建计划组状态
- 长任务不会立即生成任何子计划

**子 plan 创建（由计划组触发）**：

当计划组在 `GROUP_PLANNED` 后创建子 plan 时，子 plan 文件头部增加元数据：
```
> 所属计划组：{group_slug}
> 计划组子项：{child_id}
```
