...
- 在返回结果前先做一次 draft 自检：检查占位符未替换、章节缺失、前后矛盾、边界遗漏、验证步骤缺证据、步骤顺序不合理等问题；未通过自检时先在 subagent 内修正，再返回固定摘要协议
