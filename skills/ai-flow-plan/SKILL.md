...
- 当任务来源于 `/ai-flow-plan-review` 未通过后的回流修订时，**仍然必须先读取 `~/.config/ai-flow/setting.json` 的 `engine_mode`，再按下方委派规则选择 plan subagent；禁止因为“修订”语义直接固定指派 `ai-flow-claude-plan` 或任何其他 agent**
...
**先读取 `~/.config/ai-flow/setting.json` 中的 `engine_mode`，再决定目标 subagent。不要跳过这一步，也不得替换为任何其他 agent（包括内置 Plan agent）。**
...
Agent(
    ...
    subagent_type="<按上表选择>",
    prompt="需求描述：{需求描述}\nslug：{slug 或留空自动生成}\n\n--- 以下是 subagent 执行指令 ---\n你必须在写入 plan 之前完成以下动作：\n1. 检查当前目录 .ai-flow/rule.yaml 是否存在；若不存在且全局存在 ~/.config/ai-flow/rule.yaml，则先将其初始化到 .ai-flow/rule.yaml，并根据当前源码现状（技术栈、重要文档如 README/CLAUDE.md、是否存在测试目录等）对其中的 shared_context、required_reads 和 test_policy 进行初步填充\n2. 如果需求描述中包含文件路径（相对路径或绝对路径），先完整读取该文件内容，再将其作为原始需求原文使用\n3. 读取 plan 模板文件（templates/plan-template.md），严格按模板的章节结构和元数据格式（> 引用块）输出 plan\n4. 读取 plan 生成提示词（prompts/plan-generation.md），遵守其中的章节门禁、首行门禁、元数据门禁等硬规则\n5. 输出计划首行必须是 '# 实施计划：{一句话标题}'，头部必须包含 '> ' 引用块元数据\n6. **原始需求（原文）** 必须完整保留读取到的需求文件原文，不得改写或仅引用路径\n7. 必须包含模板定义的全部子章节（2.1-2.6、4.1-4.4、8.1-8.3），不得省略\n8. 通过 flow-state.sh 初始化状态文件，不得使用手工 JSON 操作\n9. 每个 Step（### Step N: 标题）必须按顺序包含以下 9 个子字段，不得改名、不得改变层级、不得省略：**目标**、**文件边界**（至少一行 Create/Modify/Test）、**本轮 review 预期关注面**、**前置阅读**（不需要时整段删除）、**执行动作**（- [ ] 复选框，默认 5 项：写失败用例/运行失败验证/实现最小改动/运行通过验证/本步自检）、**本步验收**、**本步关闭条件**、**阻塞条件**（无时写「无」）"
)
```
