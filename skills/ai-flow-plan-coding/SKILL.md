...
- 若更新 plan 文档前为了安全临时复制 plan 文件，允许创建 `.ai-flow/plans/{slug}.md.bak`，但该文件只能在单次操作内部临时存在；无论本次编码/修复成功、失败、中止或进入部分完成分支，返回最终协议前都必须删除本次创建的 `.bak`
- 禁止把 `.bak` 文件作为执行证据或版本历史；需要持久化历史时只能依赖 `.ai-flow/plans/history/<slug>/vN.md`
