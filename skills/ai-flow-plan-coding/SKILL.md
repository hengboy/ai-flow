...
- 部分完成 → `⚠️ Step 5 涉及跨仓库改动未完成，已记录到计划文档，状态进入 AWAITING_REVIEW。`
- 失败/被拒 → `❌ 当前状态为 AWAITING_REVIEW，需先完成代码审查后再继续。`

然后根据 `NEXT` 值追加下一步提示：

- `NEXT: ai-flow-plan-coding-review` → 输出 `下一步：运行 /ai-flow-plan-coding-review 对已完成的代码变更进行审查。`
- `NEXT: ai-flow-plan-coding` → 输出 `下一步：继续运行 /ai-flow-plan-coding 执行剩余任务。`
- `NEXT: none` → 不输出下一步提示

内部仍需在末尾追加机器可读的协议块（折叠显示，不影响阅读体验）：
