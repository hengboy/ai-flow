...
- `PLANNED`：表示计划已审核通过；先调用 `$HOME/.config/ai-flow/scripts/flow-state.sh start-execute {YYYYMMDD}-{slug}`，进入 `IMPLEMENTING`
- `IMPLEMENTING`：继续开发，不新增 transition
- `REVIEW_FAILED`：先调用 `$HOME/.config/ai-flow/scripts/flow-state.sh start-fix {YYYYMMDD}-{slug}`，进入 `FIXING_REVIEW`
