...
- **编排非交互模式**：当由 `ai-flow-plan-orchestrate` 调用时，必须视为非交互自动提交链路。遇到 pull conflict、上游缺失、验证失效、review 未覆盖当前待提交变更、额外改动不明、权限/密钥缺失或任何需要业务取舍的场景，必须失败返回；不得进入交互式取舍、不得要求用户现场选择后继续。
- **计划组 slug 拒绝**：如果 slug 匹配 `.ai-flow/plan-groups/state/{slug}.json`，拒绝并提示："第一版 /ai-flow-git-commit 不接受计划组 slug，请提交具体 DONE 子 plan 或普通 plan。"
