...
- 绑定 `slug` 时：第一执行动作必须是调用 `$HOME/.config/ai-flow/scripts/flow-bug-fix.sh {slug}`；该 runtime 入口沿用 `/ai-flow-plan-coding` 的状态门禁和执行前规则校验
...
- 开始修复前必须先确认问题是否可稳定复现，或至少能找到可靠的失败证据；若当前无法确认问题表现、影响范围或验证方式，必须先停下补充上下文，不能盲改
- 修复完成前必须有本轮新鲜验证证据；不能因为“改动看起来很小”就跳过验证并宣告已修复
- 如果收到的是 review 反馈或外部缺陷单，先理解问题与证据，再下手修改；对描述含糊、与代码事实冲突或可能牵涉需求变化的反馈，先澄清再改
