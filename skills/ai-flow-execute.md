...
- `PLANNED`：先调用 `flow-state.sh start-execute`，进入 `IMPLEMENTING`
- `IMPLEMENTING`：继续开发，不新增 transition
- `REVIEW_FAILED`：先调用 `flow-state.sh start-fix`，进入 `FIXING_REVIEW`
...
```bash
~/.claude/workflows/flow-change.sh {需求关键词} "变更描述"
```

### 4. 完成后的状态推进

- 首轮开发完成：调用 `flow-state.sh finish-implementation`，状态进入 `AWAITING_REVIEW`
- 修复完成：调用 `flow-state.sh finish-fix`，状态进入 `AWAITING_REVIEW`

### 5. 缺陷修复原则

- 失败状态只读 `last_review` / `active_fix` 追踪最近失败报告
- 可以更新旧报告中的“缺陷修复追踪”表
- 每次修复至少同时完成三件事：修主缺陷、补对应缺陷族的最小必要测试/验证、更新旧报告 `## 6. 缺陷修复追踪`
- 如果最近一次失败是 regular 第 2 轮，继续修复前必须先运行 `flow-change.sh` 补录 root cause，变更描述统一使用前缀 `[root-cause-review-loop]`
