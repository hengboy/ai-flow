...
整个流程由 **10 个 Skill** + **6 个 Subagent** + **5 个 Runtime 脚本** + **3 个共享执行器** 协作完成。
...
- `ai-flow-plan-coding`、`ai-flow-change`、`ai-flow-status`、`ai-flow-bug-fix`、`ai-flow-code-refactor`、`ai-flow-git-commit` → **直接编排**（skill 层直接编排 runtime 脚本）
...
共 10 个 Skill，每个是一个目录含 `SKILL.md`：
...
4. 运行 `ai-flow-plan-coding-review`，聚合多 repo 变更完成 review
