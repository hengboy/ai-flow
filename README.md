...
4. **Coding Review（代码审查）**：审查代码是否符合计划和质量标准

整个流程由 **9 个 Skill** + **6 个 Subagent** + **4 个 Runtime 脚本** + **3 个共享执行器** 协作完成。
...
- `ai-flow-plan-coding`、`ai-flow-change`、`ai-flow-status`、`ai-flow-bug-fix`、`ai-flow-code-refactor` → **直接编排**（skill 层直接编排 runtime 脚本）
...
| `lib/workspace-common.sh` | Workspace helper | workspace root 解析、manifest 加载、repo 枚举 |
| `plan/bin/plan-executor.sh` | Plan 执行器（1549 行） | 生成/修订 plan、状态创建、引擎 fallback |
| `plan/prompts/` | Plan 提示词 | generation、review、revision 三种 prompt |
| `plan/templates/plan-template.md` | Plan 模板 | 8 章标准计划模板 |
| `plan-review/bin/plan-review-executor.sh` | Plan 审核执行器 | 审核 plan、回写第 8 章、推进状态 |
| `coding-review/bin/coding-review-executor.sh` | 代码审查执行器（1092 行） | 生成 review 报告、缺陷推导、状态推进 |
...
- 在目标 Git 仓库根目录运行
- `.ai-flow/` 落在仓库根目录下
- 状态 schema 自动标记为 `execution_scope.mode = "single_repo"`
...
**运行约束：**
- 在 workspace 根目录运行
- 需要 `.ai-flow/workspace.json` manifest
- 所有 plan / report / state 统一落在 workspace 根目录的 `.ai-flow/` 下
...
3. 运行 `ai-flow-plan` 生成跨仓 plan
4. 运行 `ai-flow-plan-review` 审核跨仓边界与依赖
5. 运行 `ai-flow-plan-coding`，按 plan 修改多个 repo
6. 运行 `ai-flow-plan-coding-review`，聚合多 repo 变更完成 review
...
其中 `<slug>` 是流程的唯一标识符，格式为小写字母 + 数字 + 连字符（如 `user-auth-refactor`），会作为 `.ai-flow/state/<slug>.json` 文件名。
...
位于 `runtime/scripts/`，4 个确定性公共脚本：

### `flow-state.sh`（1166 行）
...
| `execution_scope` | object | 执行范围（`mode`、`workspace_file`、`repos`） |

### `flow-status.sh`（213 行）

只读状态扫描：
- 扫描 `.ai-flow/state/*.json` 按 `current_status` 分类展示
- 支持 workspace 模式（展示 repo 清单）
- 遇到坏 state 文件继续展示其他合法任务
- 输出每个状态的下一步 skill 映射

### `flow-change.sh`（124 行）
...
### `flow-workspace.sh`（219 行）

Workspace manifest 管理：

| 子命令 | 职责 |
|--------|------|
| `detect-root` | 向上查找包含 `.ai-flow/workspace.json` 的目录 |
| `validate-manifest` | 校验 schema_version、name、repos 数组 |
| `list-repos` | 输出 repo id / path |
| `repo-git-root` | 获取指定 repo 的 git root |

---

## 执行器

### Plan 执行器（`subagents/shared/plan/bin/plan-executor.sh`，1549 行）
...
### Coding Review 执行器（`subagents/shared/coding-review/bin/coding-review-executor.sh`，1092 行）

- `AWAITING_REVIEW` 走 regular review
- `DONE` 走 recheck（审查后状态仍是 `DONE` 或降级到 `REVIEW_FAILED`）
- 无 `slug` 时走 adhoc review（不推进状态，`STATE` 固定为 `none`）
...
| `test_subagent_coding_review.sh` | Regular review/recheck/adhoc、root-cause gate、引擎降级 |
...
│
├── subagents/                          # 6 个 Engine-specific Agent + Shared 资产
│   ├── ai-flow-codex-plan/
...
│   └── shared/
│       ├── lib/                        # 通用 helper（agent-common.sh / workspace-common.sh）
│       ├── plan/                       # Plan 共享资产（执行器 + prompts + 模板）
│       ├── plan-review/                # Plan Review 共享资产
│       └── coding-review/              # Coding Review 共享资产
│
├── runtime/
│   └── scripts/                        # 4 个 Runtime 脚本
│       ├── flow-state.sh               # 唯一状态写入口（1166 行）
│       ├── flow-status.sh              # 只读状态展示
│       ├── flow-change.sh              # 需求变更审计
│       └── flow-workspace.sh           # Workspace manifest 管理
│
└── tests/                              # 15 个测试文件
    ├── run.sh                          # 测试统一入口
