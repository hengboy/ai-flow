...
整个流程由 **12 个 Skill** + **6 个 Subagent** + **8 个 Runtime 脚本** + **3 个共享执行器** 协作完成。
...
- `ai-flow-git-commit` → **直接编排**（skill 层通过提示词直接驱动 git stash → pull → stash pop → add → commit 全流程，不再依赖 runtime 脚本或 subagent）
