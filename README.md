...
- `ai-flow-git-commit` → **编排 + commit subagent**（skill 层先调用 runtime 准备 repo 上下文，再委派同一个 commit subagent 完成分组与 message，最后回到 runtime 执行提交）
...
| `ai-flow-git-commit` | `skills/ai-flow-git-commit/SKILL.md` | 编排 | 在 `DONE` 后按仓库依赖顺序和业务关联性拆分提交代码，并按 runtime 产出的固定 agent input 委派 commit subagent |
...
| `ai-flow-claude-git-commit` | `subagents/ai-flow-claude-git-commit/AGENT.md` | Claude | Git 提交分组与 message 生成（`mode=group` / `mode=message`） |

**Codex vs Claude 区别：**
- Codex agent 通过 **HARD-GATE** 强制运行共享执行器脚本，不直接操作文件
- Claude agent 直接使用内置能力完成工作
- Codex 不可用时自动降级到对应的 Claude agent
- `ai-flow-claude-git-commit` 当前只有 Claude 版本，由 `ai-flow-git-commit` 原样转发 runtime 产出的 `group_agent_input` / `message_agent_input`，先按 repo 调 `mode=group`，校验后再按 group 调 `mode=message`
...
除 `ai-flow-claude-git-commit` 的 `mode=message` 外，其他 subagent 完成后都返回统一协议。该协议用于 skill/subagent 间解析、状态推进和自动化汇总，不是面向用户的主要阅读内容；最终给用户的回复应优先使用自然语言摘要和下一步提示，除非用户明确要求查看协议字段，否则不直接暴露协议块。`ai-flow-claude-git-commit` 在 `mode=group` 下只返回 JSON，在 `mode=message` 下只返回固定的 `SUBJECT/BODY/FOOTER` 结构。
...
磁盘状态文件 `.ai-flow/state/{YYYYMMDD}-{slug}.json` 固定字段：
...
- 先为每个 repo 委派 runtime 产出的 `group_agent_input` 给 `ai-flow-claude-git-commit` 生成业务分组，再为每个 group 委派 runtime 产出的 `message_agent_input` 生成结构化 message
- `--validate-groups-json` 的返回值必须作为最终提交阶段的原样输入使用，禁止重组 `groups-json`
- runtime 会持久化已校验分组原文、`message-map-json` 与提交结果；最终提交阶段失败时可通过 `--resume-commit` 恢复
...
### Commit 执行器（`runtime/scripts/flow-commit.sh`）
...
| 测试文件 | 覆盖内容 |
|----------|----------|
| `test_runtime_state.sh` | 合法/非法状态迁移、锁冲突、结构校验 |
...
├── state/{YYYYMMDD}-{slug}.json              # 流程状态（schema v4，磁盘最小 schema）
```
