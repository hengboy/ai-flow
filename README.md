...
整个流程由 **13 个 Skill** + **Claude Code agent bundle** + **Codex native custom agent** + **Runtime 脚本** + **共享执行器** 协作完成。
...
- `ai-flow-plan-coding`、`ai-flow-auto-run`、`ai-flow-plan-orchestrate`、`ai-flow-code-optimize`、`ai-flow-change`、`ai-flow-status`、`ai-flow-bug-fix`、`ai-flow-code-refactor` → **直接编排**（skill 层直接编排 runtime 脚本）
...
| `ai-flow-plan-orchestrate` | `skills/ai-flow-plan-orchestrate/SKILL.md` | 编排 | 按队列顺序执行多个已存在普通 plan，逐个 DONE 后提交并开启新会话继续 |
...
# 可选：按队列自动执行多个已有普通 plan 并逐个提交
/ai-flow-plan-orchestrate --queue <queue_slug> <plan_slug_1> <plan_slug_2> ...
```
...
`/ai-flow-plan-orchestrate` 是多 plan 队列入口，只接受用户按顺序传入的已有普通 plan slug。它逐个执行 `ai-flow-auto-run -> DONE -> ai-flow-git-commit <slug>`，每个 plan 提交完成后再通过新终端会话恢复队列并继续下一个 plan。该入口不编排计划组 children，不创建 plan，也不会让多个 plan 的未提交改动混在一起。
...
当提交由 `/ai-flow-plan-orchestrate` 触发时，`ai-flow-git-commit` 必须使用编排非交互模式。遇到 pull conflict、上游缺失、验证失效、review 未覆盖当前待提交变更、额外改动不明、权限/密钥缺失或业务取舍时，必须失败返回，由 orchestrator 写入队列 `FAILED` 并停止。
...
```bash
/ai-flow-plan-orchestrate --queue release-batch 20260609-plan-a 20260609-plan-b
```

恢复或查看：

```bash
/ai-flow-plan-orchestrate --resume release-batch
/ai-flow-plan-orchestrate --status release-batch
```

队列状态落在 `.ai-flow/orchestrations/state/{queue_slug}.json`。item 状态为 `PENDING`、`RUNNING`、`DONE_REVIEWED`、`COMMITTED`、`FAILED`；queue 状态为 `READY`、`RUNNING`、`FAILED`、`DONE`。

入队只允许普通 plan 的 `PLANNED`、`IMPLEMENTING`、`AWAITING_REVIEW`、`REVIEW_FAILED`、`FIXING_REVIEW`、`DONE` 状态，其中 `DONE` 必须仍需提交或复审。`AWAITING_PLAN_REVIEW`、`PLAN_REVIEW_FAILED`、计划组 slug、重复 slug、无效 slug、损坏 state 都会被拒绝。

失败恢复方式：

- 先查看状态：`/ai-flow-plan-orchestrate --status <queue_slug>`
- 处理失败原因，例如补权限、解决上游分支、重新审查或清理不明改动
- 确认 active item 可继续后执行：`flow-plan-orchestrate.sh --reopen-current <queue_slug> --reason "<恢复原因>"`，再 `/ai-flow-plan-orchestrate --resume <queue_slug>`

自动打开新会话由 `flow-plan-orchestrate-launch.sh` 完成。默认 `launcher=auto` 优先使用可用 `tmux`；macOS 可配置 Terminal/osascript；测试可使用 `--dry-run` 验证命令。

配置可放在用户级 `$AI_FLOW_HOME/setting.json`，项目级 `.ai-flow/setting.json` 会覆盖用户级：
...
`tool=auto` 时优先使用显式配置；未配置则按 `engine_mode` 推导；`engine_mode=auto` 时按当前宿主或可用 CLI 检测选择。该能力不硬编码只支持 Codex 或 Claude，自定义 CLI 可通过 `command_templates.custom` 接入。
...
多普通 plan 队列编排辅助脚本：
- `--queue <queue_slug> <plan_slug...>`：创建队列并校验所有普通 plan slug
- `--resume <queue_slug>`：读取队列，输出当前 active item
- `--status <queue_slug>`：只读查看队列
- 内部写入口：`--start-current`、`--mark-reviewed`、`--record-heads`、`--mark-committed`、`--fail`、`--reopen-current`

状态文件固定在 `.ai-flow/orchestrations/state/{queue_slug}.json`。该脚本不复制普通 plan state，只保存 slug 引用、item 状态、commit 前 HEAD 和提交结果。

### `flow-plan-orchestrate-launch.sh`

队列新会话启动器：
- 读取合并后的 setting（项目级覆盖用户级）
- 生成 `/ai-flow-plan-orchestrate --resume <queue_slug>` prompt
- `--dry-run` 只输出 `tool`、`launcher`、`command`、`launch_command`，不打开终端
- `launcher=auto` 默认优先使用 `tmux new-session`
...
| `tests/test_flow_plan_orchestrate.sh` / `tests/test_flow_plan_orchestrate_state.py` | 多 plan 队列编排 |
...
│       ├── flow-auto-run.sh            # 单 plan 自动编排辅助
│       ├── flow-plan-orchestrate.sh    # 多普通 plan 队列辅助
│       ├── flow-plan-orchestrate-launch.sh
